...
def main():
    ...
    mapping_info = "mapping_text_%s_video_preview_%s_intensive_%s_tag_vocab_size_%d" % (opt.text_mapping_layers, opt.visual_mapping_layers_preview, opt.visual_mapping_layers_intensive, opt.tag_vocab_size)
    ...
    opt.logger_name = os.path.join(rootpath, collections_pathname['train'], opt.cv_name, collections_pathname['val'],
                                   model_name, use_bert, num_layer, num_head, qkv_input_dim, qkv_out_dim, num_cnn,
                                   model_info, text_encode_info, visual_encode_info, mapping_info, loss_info, optimizer_info, opt.postfix)
    ...
    opt.visual_kernel_stride = list(map(int, opt.visual_kernel_stride.split('-')))
    ...
    if opt.use_bert == 1:
        opt.text_mapping_layers[0] = opt.bow_vocab_size + opt.text_rnn_size*2 + opt.text_kernel_num * len(opt.text_kernel_sizes) + 1024
    else:
    ...
    data_loaders = data.get_train_data_loaders(opt,
        caption_files, visual_feats, tag_path, tag_vocab_path, rnn_vocab, bow2vec, opt.bert_file, opt.batch_size, opt.workers, video2frames=video2frames)
    ...
    val_text_data_loader = data.get_txt_data_loader(opt, caption_files['val'], rnn_vocab, bow2vec, opt.bert_file, opt.batch_size, opt.workers)
